...
export type ToolShowcaseMode = "crop" | "smart" | "bulk" | "safezone";
...
/* ------------------------------------------------------------------ */
function Scene({ style }: { style?: CSSProperties }) {
  return (
    <svg
      viewBox="0 0 400 300"
...
function ResultPanel({
  ...
  return (
    ...
    >
      <div
        className="tks-pan"
        style={{ position: "absolute", inset: 0, animationDelay: `${delay}s` }}
      >
        <Scene />
      </div>
...
function CropShowcase({
  ...
  const frameSize: CSSProperties = landscape
    ? { width: "56%", aspectRatio: ratio }
    : { height: "80%", aspectRatio: ratio };
    ...
    <div className="flex flex-col items-center gap-5 sm:flex-row sm:justify-center sm:gap-8">
      <div>
        <div className="grid grid-cols-3 gap-2">
          {delays.map((d) => (
            <SourcePanel key={d} style={{ width: 104 }}>
              <CropFrame
                style={{ width: "58%", aspectRatio: ratio }}
                animation="tksDrift"
                delay={d}
                minimal
              />
    ...
    <section aria-label={heading}>
      ...
      <div className="mx-auto max-w-3xl rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
        {mode === "bulk" ? (
          ...
          <CropShowcase ratio={ratio} shape={shape} mode={mode} outputLabel={outputLabel} />
        )}
      </div>
    </section>
  );
